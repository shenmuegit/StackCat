...
public class MethodTrackingBridge {
	...
	private static final class CallHistoryHolder {
		// ThreadLocal data collection (lazy initialization to avoid class loading
		// issues)
	...
	private static final class SequenceCounterHolder {
		private static final ThreadLocal<AtomicInteger> SEQUENCE_COUNTER = ThreadLocal
	...
	// Configuration variables (removed static initialization to avoid class loading
	// issues)
	...
	private static class MethodCallInfo {
		...
		MethodCallInfo(String className, String methodName, int depth, int sequence, LocalDateTime callTime,
				int parentIndex, String query) {
	...
	public static void recordMethodCall(String className, String methodName, int depth, String query) {
		...
		// Log all method calls for debugging (to see if Service methods are being
		// called)
		log.debug("Recorded method call: {}.{} (depth={}, seq={}, total={})", className, methodName, depth, sequence,
				calls.size());
	}
	...
	public static void handleRepositoryMethodInvocation(Object method, Object repositoryInterface) {
		try {
			...
			// Note: If SQL is not available yet, query will be null and will be updated
			// later
	...
	public static void recordMyBatisMethodEntry(Object mappedStatement, Object parameter) {
		try {
			...
			// (in case extractAndRecordMyBatisSql was called before
			...
			// Record method call (with SQL if available, otherwise will be updated after
			// prepareStatement)
	...
	// Extract SQL from MyBatis Statement and update the last method call
	// immediately
	...
	private static String extractSqlFromPreparedStatementLogger(Object logger) {
			...
			try {
				...
				if (invocationHandler != null) {
					...
					if (invocationHandler.getClass().getName().contains("PreparedStatementLogger")) {
						...
						java.lang.reflect.Field statementField = invocationHandler.getClass()
								.getDeclaredField("statement");
						statementField.setAccessible(true);
						Object realStatement = statementField.get(invocationHandler);

						if (realStatement != null) {
							log.info("Got real statement from PreparedStatementLogger: {}",
									realStatement.getClass().getName());
							...
							if (stmtStr.contains("wrapping")) {
								// Extract SQL from string like "HikariProxyPreparedStatement@xxx wrapping
								// SELECT * FROM ..."
		...
		try {
			...
			if (realStatement != null) {
				log.info("Got real statement from PreparedStatementLogger (direct): {}",
						realStatement.getClass().getName());
	...
	private static Object unwrapHikariProxy(Object stmt) {
		try {
			...
			if (className.contains("PreparedStatementLogger")) {
				log.info(
						"Detected PreparedStatementLogger in unwrapHikariProxy, should be handled in tryExtractFromMySQL or extractSqlFromPreparedStatementLogger");
	...
	// Note: Parameter is Object to match bytecode signature (ServletRequest is
	// Object at runtime)
	public static Map<String, String> extractHeadersFromRequest(Object request) {
		...
		try {
			...
			// Directly try to call getHeaderNames and getHeader methods on the request
			// object
	...
	private static void sendBatch(List<BatchData> batch) {
		...
		try {
			...
			if (response.statusCode() >= 200 && response.statusCode() < 300) {
				...
				log.error("Failed to send tracking data: HTTP {}, Response: {}", response.statusCode(),
						response.body());
			}
